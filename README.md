...
var teamS;
const AdminsAuths = [
    "5-rB5vrsRNWxa_HsQS4tZA5Shu0EJS7JUF2NuIEf2tU",
    "qHwiJ6BMCyQm4NUS5u9Wof_8vUvVAiGJkv1owOwm2eQ",
    "9m8vYbsuLWzXmQDz41HQEJFEg-GH0V-R0Ki73WxM-LI"
];
...
//KITS//
let predefinedKits = {
    strong: { name: "The Strongest", home: { angle: 0, colors: ["FFFFFF", "FFC60A", "000000", "FFC60A"] }, away: { angle: 60, colors: ["EB7500", "300F00"] } },
    always: { name: "Always Ready", home: { angle: 30, colors: ["7D1601", "FFFCFC", "F50000", "FFFFFF"] }, away: { angle: 60, colors: ["3B0000", "850000", "FFFFFF", "850000"] } },
    bolivar: { name: "Bolívar", home: { angle: 30, colors: ["FFC919", "21B7ED", "1AA0ED", "21B7ED"] }, away: { angle: 30, colors: ["FFC919", "004734", "00331F", "004734"] } },
    blooming: { name: "Blooming", home: { angle: 30, colors: ["FDFFFC", "00CCFF", "0DB6FF", "00CCFF"] }, away: { angle: 30, colors: ["FDFFFC", "FF8A05", "FFAE21", "FF8A05"] } },
    gvsj: { name: "GV San José", home: { angle: 30, colors: ["030336", "FFFFFF", "E3E3E3", "FFFFFF"] }, away: { angle: 30, colors: ["FFFFFF", "041B59", "030336", "041B59"] } },
    guabira: { name: "Guabirá", home: { angle: 30, colors: ["FFFFFF", "590000", "360000", "590000"] }, away: { angle: 0, colors: ["990005", "050259", "001036"] } },
    sanantonio: { name: "San Antonio", home: { angle: 0, colors: ["E8E8E8", "1BE8F7", "FFFFFF", "1BE8F7"] }, away: { angle: 0, colors: ["FFFFFF", "052424", "073333"] } },
    inde: { name: "Independiente Petrolero", home: { angle: 60, colors: ["FFFFFF", "C20000", "990000", "C20000"] }, away: { angle: 60, colors: ["BF0600", "FFFFFF", "E6E6E6", "FFFFFF"] } },
    naci: { name: "Nacional Potosí", home: { angle: 30, colors: ["000000", "FFFFFF", "AD0000", "FFFFFF"] }, away: { angle: 30, colors: ["000000", "AD0000", "FFFFFF", "AD0000"] } },
    realoruro: { name: "Real Oruro", home: { angle: 30, colors: ["FCC203", "590000", "360000", "590000"] }, away: { angle: 30, colors: ["FFC219", "000000", "1F0000", "000000"] } },
    univinto: { name: "Universitario de Vinto", home: { angle: 30, colors: ["8F0000", "FFFFFF", "F7F7F7", "FFFFFF"] }, away: { angle: 0, colors: ["8F0000", "000000", "141414"] } },
    tomayapo: { name: "Real Tomayapo", home: { angle: 0, colors: ["1B8F70", "092B28", "0F4A24"] }, away: { angle: 330, colors: ["FAFAFA", "122927", "331515", "122927"] } },
    oriente: { name: "Oriente Petrolero", home: { angle: 60, colors: ["FFFFFF", "062914", "367A3C", "062914"] }, away: { angle: 60, colors: ["0B4210", "FFFAFA", "367A3C", "FFFFFF"] } },
    abb: { name: "ABB", home: { angle: 60, colors: ["1C1C7A", "FFFF00", "DCF500"] }, away: { angle: 60, colors: ["FFFF00", "1C1C7A", "2828AD"] } },
    wilstermann: { name: "Wilstermann", home: { angle: 60, colors: ["FFA200", "B80000", "780000", "B80404"] }, away: { angle: 60, colors: ["FFFFFF", "B80000", "230D78"] } },
    aurora: { name: "Aurora", home: { angle: 60, colors: ["FFFFFF", "05B1E6"] }, away: { angle: 60, colors: ["FF4D00", "FFBA6B"] } }
};
let currentKits = { red: null, blue: null };

function applyKit(team, kitVariant) {
    const textColor = parseInt(kitVariant.colors[0], 16);
    const avatarColors = kitVariant.colors.slice(1).map(c => parseInt(c, 16));
    room.setTeamColors(team, kitVariant.angle, textColor, avatarColors);
}

function setRandomKits() {
    let keys = Object.keys(predefinedKits);
    let redKey = keys[Math.floor(Math.random() * keys.length)];
    let blueKey;
    do { blueKey = keys[Math.floor(Math.random() * keys.length)]; } while (blueKey === redKey);

    currentKits.red = predefinedKits[redKey];
    currentKits.blue = predefinedKits[blueKey];

    applyKit(1, currentKits.red.home);
    applyKit(2, currentKits.blue.away);

    room.sendAnnouncement(`🎽 Equipos: 🟥 ${currentKits.red.name} vs 🟦 ${currentKits.blue.name}`, null, 0x87CEEB, "bold", 2);
}
...
function PublicitaScript(player) {
    ...
    room.sendAnnouncement("                                      💬  https://haxballbolivia.mgx.world/ ⬅⬅⬅ Script", null, 0xF6FF43, "bold");
    room.sendAnnouncement("                                      💬          HECHO POR HAXNALL ⬅⬅⬅ CC", null, 0xF6FF43, "bold");
...
function endGame(winner) { // handles the end of a game : no stopGame function inside //
    ...
    let redName = currentKits.red.name;
    let blueName = currentKits.blue.name;
    room.sendAnnouncement("════════════════════════════════════════", null, 0xFFD700, "bold", 2);
    room.sendAnnouncement("                                   𝐅𝐈𝐍𝐀𝐋 𝐃𝐄𝐋 𝐏𝐀𝐑𝐓𝐈𝐃𝐎", null, 0xFFD700, "bold", 2);
    room.sendAnnouncement(`📊 ${redName} ${scores.red} - ${scores.blue} ${blueName} | ⏱️ ${getTime(scores)}`, null, 0xFFFFFF, "bold", 2);
    if (winner == Team.RED) {
        streak++;
        room.sendAnnouncement(
            "🔴 " + redName + " ganó " + scores.red + "-" + scores.blue + " ! Victorias consecutivas: " + streak + " 🏆",
            null, 0xFF250D, "bold"
        );
    }
        ...
        streak = 1;
        room.sendAnnouncement(
            "🔵 " + blueName + " ganó " + scores.blue + "-" + scores.red + " ! Victorias consecutivas: " + streak + " 🏆",
            null, 0x3075FF, "bold"
        );
    }
    else {
        streak = 0;
        room.sendAnnouncement("💤 Límite de empate, terminado! 💤", null, 0xFFFFFF, "bold");
    }
    room.sendAnnouncement(
        "⭐ Posesión del balón: 🔴 " + (Rposs * 100).toPrecision(3) + "% : " + (Bposs * 100).toPrecision(3) + "% 🔵",
        null, 0x73EC59, "bold"
    );
    if (scores.red == 0 && scores.blue == 0) {
        room.sendAnnouncement("🏆 " + GKList[0].name + " y " + GKList[1].name + " mantuvieron la valla invicta!", null, 0xFFD700, "bold");
    } else if (scores.red == 0) {
        room.sendAnnouncement("🏆 " + GKList[1].name + " mantuvo la valla invicta!", null, 0xFFD700, "bold");
    } else if (scores.blue == 0) {
        room.sendAnnouncement("🏆 " + GKList[0].name + " mantuvo la valla invicta!", null, 0xFFD700, "bold");
    }
    updateStats();
    room.sendAnnouncement("════════════════════════════════════════", null, 0xFFD700, "bold", 2);
}
...
function updateRoleOnPlayerOut() {
    ...
    if (inChooseMode) {
        if (players.length < 6) {
            loadMap(x1Map, scoreLimitx1, timeLimitx1);
        }
...
function pad(str, length) {
    str = str.toString();
    while (str.length < length) str += " "; // agrega espacios al final
    return str;
}
...
room.onPlayerJoin = function (player) {
    ...
    room.sendAnnouncement("═══════════════════════════════════════════════════", player.id,0xFF0000);
    room.sendAnnouncement("¡𝗖𝗵𝗲! 𝗕𝗶𝗲𝗻𝘃𝗲𝗻𝗶𝗱𝗼 " + player.name + " 𝗔 𝗙𝘂𝘁𝘀𝗮𝗹 𝗕𝗼𝗹𝗶𝘃𝗶𝗮!", null, 0xFFE712,"bold");
    room.sendAnnouncement("👑 ¡𝗔 𝗷𝘂𝗴𝗮𝗿 𝗽𝗮! (𝗛𝗼𝘀𝘁 𝗱𝗲 𝗽𝗿𝘂𝗲𝗯𝗮)", player.id,0xFFE712,"bold");
    room.sendAnnouncement("═══════════════════════════════════════════════════", player.id, 0x1B6300);
    ...
    if (localStorage.getItem(player.auth) !== null) {
        ...
        let playerRole = playerData[Ss.RL]; // Ss.RL debe ser el índice del rol

         // Condición 1: tiene rol admin/master
         if (playerRole === "admin" || playerRole === "master") {
            room.setPlayerAdmin(player.id, true);
            room.sendAnnouncement(
                "Admin「" + player.name + "」 ¡𝗘𝗻𝘁𝗿𝗼 𝗮 𝗽𝗼𝗻𝗲𝗿 𝗼𝗿𝗱𝗲𝗻 𝗲𝗻 𝗹𝗮 𝘀𝗮𝗹𝗮!",
                null,
                0xFF7900,
                "bold"
             );
    ...
    // Condición 2: su auth está en la lista de admins
    if (AdminsAuths.includes(player.auth)) {
        room.setPlayerAdmin(player.id, true);
        room.sendAnnouncement(
            "Admin「" + player.name + "」 ¡𝗘𝗻𝘁𝗿𝗼 𝗮 𝗽𝗼𝗻𝗲𝗿 𝗼𝗿𝗱𝗲𝗻 𝗲𝗻 𝗹𝗮 𝘀𝗮𝗹𝗮!",
            null,
...
room.onPlayerChat = function (player, message) {
    ...
    if (["!help", "!ayuda"].includes(message[0].toLowerCase())) {
	    ...
	    room.sendAnnouncement(player.name + (getAFK(player) ? " Se puso a dormir💤 !" : " Desperto !"),null,(getAFK(player) ? 0xFF7B08 : 0x8FFF8F));
    ...
    return false;
        ...
        room.sendChat(cstm, player.id);
            ...
            // Cabecera
            room.sendAnnouncement("[📄] Stats de " + player.name, player.id, 0x73EC59, "bold");
            // Fila 1
            let row1 = "🎮 PJ: " + pad(stats[Ss.GA], 3) +
                       " │ ✅ W: " + pad(stats[Ss.WI], 3) +
                       " │ ❌ L: " + pad(stats[Ss.LS], 3) +
                       " │ WR: " + pad(stats[Ss.WR], 5) + "%";
            // Fila 2
            let row2 = "⚽️ GL: " + pad(stats[Ss.GL], 3) +
                       " │ 👟 AS: " + pad(stats[Ss.AS], 3) +
                       " │ 🤚 GK: " + pad(stats[Ss.GK], 3) +
                       " │ 🧱 CS: " + pad(stats[Ss.CS], 3) +
                       " (" + pad(stats[Ss.CP], 5) + "%)";

            room.sendAnnouncement(row1, player.id, 0x73EC59, "bold");
            room.sendAnnouncement(row2, player.id, 0x73EC59, "bold");
            
            room.sendAnnouncement("「👓」 Solo lo ves vos. Usa '!mostrarstats' para compartirlo.", player.id, 0xFF7900, "bold");
    }
        ...
        room.sendAnnouncement("[📄] Stats de " + player.name, null, 0x73EC59, "bold");
        ...
        room.sendAnnouncement(row1, null, 0x73EC59, "bold");
        room.sendAnnouncement(row2, null, 0x73EC59, "bold");
    ...
    if (getMute(player)) {
        room.sendChat("estás muteado.", player.id);
        return false;
    }
    if (slowMode > 0) {
        if (!player.admin) {
    ...
    if (localStorage.getItem(getAuth(player))){
        ...
        if (stats[Ss.WI] > 399){
            ...
            announcement += "👑「Martins」"
            chatColor = "0xDAA520"
        } else if (stats[Ss.WI] > 129){
            announcement += "💎「Sánchez」"
            chatColor = "0xB9F2FF"
        } else if (stats[Ss.WI] > 89){
            announcement += "🏆「Baldivieso」"
            chatColor = "0x9ACD32"
            ...
            announcement += "🎯「Ramallo」"
            chatColor = "0x3CB371"
        } else if (stats[Ss.WI] > 34){
            announcement += "⚽「Monteiro」"
            chatColor = "0xFE2E2E"
        } else if (stats[Ss.WI] > 24){
            announcement += "🔴「Torrico」"
            chatColor = "0xFF0000"
        } else if (stats[Ss.WI] > 14){
            announcement += "🟡「Medina」"
            chatColor = "0xFFFF00"
        } else if (stats[Ss.WI] > 4){
            announcement += "🎖️「Paniagua」"
            chatColor = "0x0000FF"
        } else {
            announcement += "🌱「Debutante」"
            chatColor = "0x90EE90"
        }
...
room.onTeamGoal = function (team) {
    ...
    const scores = room.getScores();
    game.scores = scores;
    let redName = currentKits.red ? currentKits.red.name : "🟥 ROJO";
    let blueName = currentKits.blue ? currentKits.blue.name : "🟦 AZUL";
    room.sendAnnouncement("════════════════════════════════════════", null, 0xFFD700, "bold", 2);
    room.sendAnnouncement(`📊 ${redName} ${scores.red} - ${scores.blue} ${blueName} | ⏱️ ${getTime(scores)}`, null, 0xFFFFFF, "bold", 2);
